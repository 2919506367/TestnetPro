...
function rewriteUrls(html: string, proxyBase: string): string {
  const attrs = ["src", "href", "action", "srcset", "data-src", "data-href", "poster", "background", "formaction", "manifest", "ping"];
  const pattern = new RegExp(`\\b(${attrs.join("|")})\\s*=\\s*("([^"]*)"|'([^']*)'|([^\\s>"'=]+))`, "gi");

  return html.replace(pattern, (full, _attr, _quoted, dq, sq, unq) => {
    const raw = (dq || sq || unq || "").trim();
    if (!raw) return full;
    if (/^(data:|javascript:|mailto:|#|blob:|ws:|about:|chrome-extension:|moz-extension:)/i.test(raw)) return full;
    if (raw.includes("/api/proxy")) return full;
    const q = dq !== undefined ? '"' : sq !== undefined ? "'" : "";
    return `${_attr}=${q}${proxyBase}?url=${encodeURIComponent(raw)}${q}`;
  });
}
