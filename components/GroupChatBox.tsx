...
export default function GroupChatBox({ groupId, replyTarget, onReplyClear, onMessageSent }: Props) {
  ...
  useEffect(() => {
    const draft = localStorage.getItem(`chat-draft-group-${groupId}`);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const body: Record<string, unknown> = { groupId, content: trimmed };
      ...
      if (res.ok) {
        setText("");
        localStorage.removeItem(`chat-draft-group-${groupId}`);
        onReplyClear?.();
        onMessageSent?.();
      }
  ...
  const handleChange = (val: string) => { setText(val); localStorage.setItem(`chat-draft-group-${groupId}`, val); };
    ...
    <div className="border-t border-gray-200/60 bg-white/60 backdrop-blur-md p-3">
      ...
      <div className="flex items-end gap-2">
        ...
        <textarea ref={inputRef} value={text} onChange={(e) => handleChange(e.target.value)} onKeyDown={handleKeyDown}
          placeholder="输入消息... (Enter发送)" rows={1} maxLength={2000}
