...
files=[
    'lib/email.ts','lib/captcha.ts','lib/verification.ts',
    'app/api/auth/captcha/route.ts','app/api/auth/send-verify-code/route.ts',
...
for name,h,pw in [('HK','38.92.9.169',pw_hk),('CN','106.14.126.214',pw_cn)]:
    ...
    for f in files:
        ...
        print(f'  {f}')
    sf.close()
    s.exec_command('cd /root/cloud-drive && npm install 2>&1 | tail -3',timeout=60)
    ...
    stdin,stdout,stderr2=s.exec_command('curl -s http://localhost:3000/api/auth/captcha 2>&1 | head -c 60',timeout=10)
    print(f'  captcha: {stdout.read().decode().strip()[:60]}')
    s.close()
print('Done')
